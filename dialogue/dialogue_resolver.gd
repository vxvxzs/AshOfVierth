...
const DIALOGUE_PATHS := {
	...
	"vierth_gate": "res://data/dialogue/vierth_gate.json"
}

static func get_lines(dialogue_id: String, state: Node = null) -> Array:
	var dialogue := get_dialogue(dialogue_id, state)
	return Array(dialogue.get("lines", [])).duplicate(true)


static func get_dialogue(
	dialogue_id: String,
	state: Node = null,
	memory: Node = null
) -> Dictionary:
...
static func _get_runtime_dialogue_manager() -> Node:
	var scene_tree: SceneTree = Engine.get_main_loop() as SceneTree
	if scene_tree == null:
		return null
	return scene_tree.root.get_node_or_null("DialogueManager")
...
static func _matches_conditions(
	...
	for flag_id in Array(conditions.get("flags_none", [])):
		if state.has_flag(StringName(flag_id)):
			return false
	var memory_equals: Dictionary = conditions.get("memory_equals", {})
	if not memory_equals.is_empty() and memory == null:
		return false
	for memory_path: String in memory_equals:
		var path_parts := memory_path.split(".", false, 1)
		if path_parts.size() != 2:
			return false
		var remembered: Variant = memory.recall(
			StringName(path_parts[0]),
			StringName(path_parts[1])
		)
		if remembered != memory_equals[memory_path]:
			return false
	return true
