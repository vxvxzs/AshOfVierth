...
enum State { CHASE, TELEGRAPH, RECOVERY, STAGGER }
...
func take_damage(amount: int) -> void:
	if has_reported_defeat:
		return
	health -= amount
	flash_time_left = 0.16
	state = State.STAGGER
	state_time_left = 0.16
	if health <= 0:
...
func reset_for_reconstruction() -> void:
	health = max_health
	state = State.CHASE
	state_time_left = 0.0
	has_hit_this_swing = false
	has_reported_defeat = false
	knockback_velocity = Vector2.ZERO
	reconstructed_once = true
	_disable_attack_hitbox()
	queue_redraw()
...
func _on_attack_area_entered(area: Area2D) -> void:
	...
	if target.has_method("is_parrying") and target.is_parrying():
