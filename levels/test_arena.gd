...
@onready var reconstruction_overlay = $ReconstructionOverlay
...
func _ready() -> void:
	...
	mobile_controls.parry_requested.connect(player.request_parry)
	reconstruction_overlay.reconstruction_finished.connect(_on_reconstruction_finished)
	ash_well.activate()
...
func _on_player_died() -> void:
	...
	if last_damage_source != null and is_instance_valid(last_damage_source) and last_damage_source.has_method("reset_for_reconstruction"):
		last_damage_source.reset_for_reconstruction()
		ProgressionManager.set_objective("Reclaim the memory from %s." % last_damage_source.display_name)
	reconstruction_overlay.show_reconstruction()
	shake_time_left = 0.16

func _on_player_damaged(source: Node) -> void:
	last_damage_source = source

func _on_reconstruction_finished() -> void:
	player.respawn(active_spawn_position)
...
func _draw() -> void:
	...
	var controls_text := "Touch: move | STRIKE: attack | GUARD: parry" if _is_mobile_platform() else "WASD / Arrow Keys: move | J, Z or left mouse: attack | K: parry"
